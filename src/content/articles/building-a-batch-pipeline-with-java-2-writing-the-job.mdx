...
slug: building-a-batch-pipeline-with-java-2-writing-the-job
excerpt: >-
  The first post in this series went over some fundamental concepts and
  interfaces you will encounter as a developer using Spring Batch. Now, let's
  build on...
date: '2022-03-07'
category: Backend
tags:
...
The job will ingest everything in a site on StackExchange (posts, comments, users, etc) into a MySQL database. Using Spring Batch we can create a maintainable and extensible Job using a minimal amount of code.

![Batch job dataflow](/images/articles/batch-job.png)
...
Out of the box there are some sample readers and writers in the package `com.batch.config.readers` for testing that aren't needed, so clear those out. With a blank project, let's work on reading and printing the data we've downloaded.
...
You may also notice the rapid-starter & package `com.batch.config.db` sets up spring-data-jpa. This will come in handy shortly... To verify successful configuration, go ahead and start up the application. You should see some new output this time similar to below if everything worked:
...
Re-run with these changes and observe that the step will write Posts to MySQL!
...
![Select * from posts](/images/articles/selectPosts.png)
...
Now we can sit back and execute the job to get an import of an entire Stack Exchange site into MySQL!
