...
excerpt: >-
  The last post took a look at a Spring Batch project template to get off the
  ground quickly. In this series we're going to take it a step further and use
  the...
date: '2021-06-04'
category: Backend
tags:
...
The last post took a look at a Spring Batch project template to get off the ground quickly. In this series we're going to take it a step further and use the starter to build out a batch job. We won't focus too much here on the implementation of the framework itself ([docs do a good job of that](https://docs.spring.io/spring-batch/docs/4.3.x/reference/html/domain.html#domainLanguageOfBatch)), but rather practical descriptions and use-cases to quickly enable yourself to build robust batch jobs for enterprise apps.
...
![Spring batch functional architecture](/images/articles/spring-batch-architecture.png)
...
You simply define a class that implements the `Tasklet` interface. This interface has a single method that needs to be defined, `execute(...)`, that is called repeatedly until it returns `RepeatStatus.FINISHED`. A common pattern in the early/late steps of a job is to run some DDL to clear a schema before writing. This could be done in a `Tasklet`:
...
It operates on an input object of type `I` and returns an object `O`. This allows for transformations if necessary. To illustrate how to define an `ItemProcessor`, let's return to the previous scenario of a CSV of names. In this scenario, we want to capitalize all the names. This is a perfect (simple) example of some business logic that could live in a processor!
