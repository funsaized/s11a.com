...
function checkBundleSizes() {
  ...
  console.log(
    totalJS <= PERFORMANCE_BUDGETS.TOTAL_JS
      ? "✅ Within budget"
      : "⚠️ Over budget",
  );
  ...
  console.log(
    totalCSS <= PERFORMANCE_BUDGETS.CSS ? "✅ Within budget" : "⚠️ Over budget",
  );
}
...
function checkImageOptimization() {
    ...
    files.forEach((file) => {
      if (file.isDirectory()) {
        ...
        const size = getFileSize(
          path.join(dir, file.name).replace(path.join(__dirname, ".."), ""),
        );

        if (file.name.includes(".webp") || file.name.includes(".avif")) {
          optimizedImages++;
          console.log(
            `✅ ${dirName}/${file.name}: ${formatBytes(size)} (modern format)`,
          );
        } else if (size > 500 * 1024) {
          console.log(
            `⚠️ ${dirName}/${file.name}: ${formatBytes(size)} (consider optimization)`,
          );
        } else {
  ...
  console.log(
    `Modern formats (WebP/AVIF): ${optimizedImages} (${Math.round((optimizedImages / totalImages) * 100)}%)`,
  );

  if (optimizedImages / totalImages < 0.5) {
    console.log(
      "⚠️ Consider converting more images to modern formats (WebP/AVIF)",
    );
  } else {
...
function checkPerformanceFeatures() {
  ...
  const features = [
    ...
    },
    {
      ...
      implemented:
        checkFileExists("gatsby-config.ts") &&
        fs
          .readFileSync(path.join(__dirname, "..", "gatsby-config.ts"), "utf8")
    ...
    },
    {
      ...
      implemented:
        checkFileExists("src/layout/index.tsx") &&
        fs
          .readFileSync(
            path.join(__dirname, "..", "src/layout/index.tsx"),
            "utf8",
          )
...
function generateRecommendations() {
  ...
  console.log(
    `• Initial JS budget: ${formatBytes(PERFORMANCE_BUDGETS.INITIAL_JS)}`,
  );
  console.log(
    `• Total JS budget: ${formatBytes(PERFORMANCE_BUDGETS.TOTAL_JS)}`,
  );
