...
const downloadAndProcessImage = async (imageUrl, outputPath, fileName) => {
  try {
    ...
    verbose(
      `✓ Processed ${processedImages.length} image variants for ${fileName}`,
    );
  ...
  try {
    ...
    for (let i = 0; i < mdxFiles.length; i += CONFIG.maxConcurrent) {
      ...
      const results = await Promise.allSettled(
        batch.map((file) =>
          processImagesInFile(path.join(CONFIG.contentDir, file)),
        ),
      );
    ...
    log(
      `Image processing complete: ${processedCount} successful, ${errorCount} errors`,
