...
// Configuration
const CONFIG = {
  sourceDir: '/Users/saiguy/Documents/programming/snimmagadda1/s11a.com/content',
  outputDir: path.join(__dirname, '../../src/content/articles'),
  imageSourceDir: '/Users/saiguy/Documents/programming/snimmagadda1/s11a.com/content',
...
const processImageReferences = (content, sourceDir) => {
  ...
  processedContent = processedContent.replace(/\.\.\/images\//g, '/images/articles/');
  
  // Replace ../thumbnails/ with /images/articles/thumbnails/
  processedContent = processedContent.replace(/\.\.\/thumbnails\//g, '/images/articles/thumbnails/');
...
const copyAllImages = async () => {
  try {
    ...
    try {
      ...
      log(`Copying ${files.length} thumbnails from content directory...`, 'info');
...
const migrateArticle = async (article) => {
  try {
    ...
    const processedContent = processImageReferences(article.content, article.sourceDir);
...
`;
    ...
    if (!CONFIG.dryRun) {
      ...
      if (!CONFIG.force) {
        try {
          ...
          log(`Skipping existing file: ${fileName} (use --force to overwrite)`, 'warn');
          return false;
        } catch (error) {
    ...
    log(`Error migrating ${article.frontmatter.title}: ${error.message}`, 'error');
    return false;
  }
};
...
const validateContent = async () => {
  try {
    ...
    log(`Validation complete: ${validCount} valid, ${errorCount} errors`, 
...
const runMigration = async () => {
  try {
    ...
    log(`Migration complete: ${successCount} successful, ${errorCount} errors`, 
        errorCount > 0 ? 'warn' : 'success');
