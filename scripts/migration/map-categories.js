    ...
    for (const file of mdxFiles) {
      ...
      if (frontmatter.tags && Array.isArray(frontmatter.tags)) {
        frontmatter.tags.forEach((tag) => {
          ...
          analytics.tags[normalizedTag] =
            (analytics.tags[normalizedTag] || 0) + 1;
        });
      }
...
const generateRecommendations = () => {
    ...
    tagKeys.forEach((tag2) => {
      if (tag1 !== tag2 && !duplicates.has(tag1) && !duplicates.has(tag2)) {
        ...
        if (
          ...
          duplicates.add(tag1);
          duplicates.add(tag2);
          analytics.duplicateTags.push({
            tag1,
            tag2,
            count1: analytics.tags[tag1],
            count2: analytics.tags[tag2],
          });
        }
      }
    });
  ...
  const unstandardizedCategories = Object.keys(analytics.categories).filter(
    (cat) => !Object.values(CATEGORY_STANDARDIZATION).includes(cat),
  );
...
const generateTextReport = (report) => {
  ...
  Object.entries(report.categories)
    ...
    .forEach(([category, count]) => {
      const percentage = ((count / report.summary.totalArticles) * 100).toFixed(
        1,
      );
    ...
    for (const file of mdxFiles) {
      ...
      if (frontmatter.category) {
        ...
        const standardizedCategory =
          CATEGORY_STANDARDIZATION[oldCategory.toLowerCase()] || oldCategory;
      ...
      if (frontmatter.tags && Array.isArray(frontmatter.tags)) {
        ...
        frontmatter.tags = frontmatter.tags.map(
          (tag) => TAG_STANDARDIZATION[tag.toLowerCase()] || tag,
        );
