...
const validateFrontmatter = (frontmatter, fileName) => {
  ...
  if (frontmatter.excerpt) {
    if (frontmatter.excerpt.length < 50) {
      addIssue(
        fileName,
        "frontmatter",
        "Excerpt is too short (minimum 50 characters)",
        "warning",
      );
    }
    if (frontmatter.excerpt.length > 200) {
      addIssue(
        fileName,
        "frontmatter",
        "Excerpt is too long (maximum 200 characters)",
        "warning",
      );
    }
  }
  ...
  if (
    ...
    addIssue(
      fileName,
      "frontmatter",
      `Invalid category: ${frontmatter.category}`,
      "error",
    );
    isValid = false;
  }
...
const validateContent = (content, fileName) => {
  ...
  if (wordCount < 300) {
    addIssue(
      fileName,
      "content",
      `Content is too short (${wordCount} words, minimum 300)`,
      "warning",
    );
  }
  ...
  codeBlocks.forEach((block, index) => {
    ...
    if (firstLine === "```") {
      addIssue(
        fileName,
        "content",
        `Code block ${index + 1} missing language specification`,
        "warning",
      );
    }
  });
...
const validateSEO = (frontmatter, content, fileName) => {
  ...
  if (
    ...
    addIssue(
      fileName,
      "seo",
      "Title length not optimal for SEO (30-60 characters recommended)",
      "warning",
    );
  }
  ...
  if (
    frontmatter.excerpt &&
    (frontmatter.excerpt.length < 120 || frontmatter.excerpt.length > 160)
  ) {
    addIssue(
      fileName,
      "seo",
      "Excerpt length not optimal for meta description (120-160 characters recommended)",
      "warning",
    );
  }
  ...
  if (frontmatter.tags && frontmatter.tags.length > 0) {
    ...
    if (!titleLower.includes(focusKeyword)) {
      addIssue(
        fileName,
        "seo",
        `Focus keyword "${focusKeyword}" not in title`,
        "warning",
      );
    }
    ...
    if (!contentLower.includes(focusKeyword)) {
      addIssue(
        fileName,
        "seo",
        `Focus keyword "${focusKeyword}" not in content`,
        "warning",
      );
    }
  }
  ...
  try {
    ...
    if (!frontmatterMatch) {
      addIssue(
        fileName,
        "structure",
        "Invalid file structure - missing frontmatter",
        "error",
      );
      return false;
    }
...
const generateReport = () => {
  ...
  log(
    ...
    validationResults.errors > 0 ? "error" : "success",
  );
  log(
    `Warnings: ${validationResults.warnings}`,
    validationResults.warnings > 0 ? "warn" : "info",
  );
  ...
  // Summary
  const successRate = (
    ((validationResults.files - validationResults.errors) /
