...
- Install Tailwind CSS via Gatsby plugin. IMPORTANT: FOLLOW THESE INSTALLATION STEPS https://v3.tailwindcss.com/docs/guides/gatsby
- Configure PostCSS and Tailwind
- Set up shadcn/ui with Amber palette: IMPORTANT: follow these installation steps https://ui.shadcn.com/docs/installation/gatsby
- Create new global styles

#### **Phase 2: Component Migration (Day 3-5)**
- Install shadcn components progressively
- Migrate atomic components (buttons, inputs). IMPORTANT: USE THE SHADCN CLI TO GENERATE AND ADD COMPONENTS. NEVER COPY PASTE CODE.
- Update composite components (navbar, cards)
- Add Sidebar from shadcn and implement Collapsible sidebar on all layouts that use the Sidebar component and "pushes" the content to the right on open.
...
- Optimize CSS usage, remove custom CSS, prefer re-use, built in colors https://ui.shadcn.com/colors
- Update imports across all components
- Use playwright MCP to verify dev and production builds look identical and function
...
Use the shadcn cli to add components. always reference the shadcn-ui MCP when working with shadcn.
